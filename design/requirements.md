...
- **R79:** (inferred) Before opening the console the panel states that the bookmarklet must be clicked again on the page about to appear, and withholds its acknowledgement control long enough for that to be read rather than clicked past; the console opens only on acknowledgement, because once it is open the app has no channel to the player at all — measured, and not for want of trying: a separate window cannot raise itself, so it is buried by the tab it opens.
