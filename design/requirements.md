...
- **R79:** (inferred) Before opening the console the panel states that the bookmarklet must be clicked again on the page about to appear, and withholds its acknowledgement control long enough for that to be read rather than clicked past; the console opens only on acknowledgement, because once it is open the app has no channel to the player at all. (Provisional — see gap O5: a surviving popup instruction surface would falsify the no-channel premise and rewrite this requirement's justification and probably its delay.)
- **R80:** (inferred) The panel remains on the app page after the console opens, carrying that standing instruction, so a player who backs out to the tab they came from finds it rather than an unchanged screen. (Provisional — see gap O5: which surface carries the standing instruction depends on the probe's result.)
